...
const fileReducer = (state, action) => {
  switch (action.type) {
    ...
    case UPLOAD_FILE:
      ...
      const sumOfSize = parseFloat(
        (state.sumOfSize + action.payload.fileSize / 1024).toFixed(1)
      )
      return {
        ...state,
        isUploading: true,
    ...
    case SELECT_TO_COMPARE:
      return {
        ...state,
        selectedFilesToCompare: state.selectedFilesToCompare.concat(
          action.payload
        ),
    ...
    case REMOVE_COMPARE_FILE:
      return {
        ...state,
        selectedFilesToCompare: state.selectedFilesToCompare.filter(
