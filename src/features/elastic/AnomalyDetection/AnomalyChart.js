...
const AnomalyChart = ({data}) => {const today = new Date();
    ...
    options: {
      ...
      tooltip: {
        custom: function({ series, seriesIndex, dataPointIndex, w }) {
			...
			return "<div class='tooltip'>" + '<ul>' +
			'<li><b>' + data.x + '</b></li>' +
			'<li class="bold" ><b>Score</b>:' + data.y[2] + '</li>' +
			'</ul>' + "</div>";
        }
      }
    }
  ...
  return (
    <div>
      <div id="chart">
	  {data && data.length > 0 && <ReactApexChart options={chartData.options} series={chartData.series} type="rangeBar" height={100} />}
