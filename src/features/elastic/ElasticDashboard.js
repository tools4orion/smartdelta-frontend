...
import { Card, Grid, Icon, IconButton, Tab, Tabs, Typography } from "@mui/material";
...
const handleMlSetup = async() =>{
  ...
  const renderSparklineChart = (data, title, yaxisLabel) => {
    const seriesData = data.map(entry => ({
      ...
      formattedDate: new Date(entry.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
    }));
    ...
    <DashboardLayout>
      ...
      >
		...
		<Grid item xs={12} md={4}>
            {dataState && renderSparklineChart(dataState.sparklineData.functionDuration, 'Function Duration', 'Duration')}
          </Grid>
          <Grid item xs={12} md={4}>
            {dataState && renderSparklineChart(dataState.sparklineData.billedDuration, 'Billed Duration', 'Billed Duration')}
          </Grid>
          <Grid item xs={12} md={4}>
            {dataState && renderSparklineChart(dataState.sparklineData.memoryUsage, 'Memory Usage', 'Free Memory')}
          </Grid>
	</CustomTabPanel>
