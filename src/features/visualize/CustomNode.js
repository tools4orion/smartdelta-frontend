...
export default memo(({ data, tableData }) => {
  ...
  const { state, dispatch, toggleSidePanel, toggleLatencySidebar, selectNode } =
    ...
    <>
      ...
      >
        <MDBox p={1} mt={4} sx={bgStyles}>
          <MDTypography variant="h6" sx={{ p: 1 }}>
            Anomaly Detection <InfoIcon fontSize="small" />{" "}
          </MDTypography>

          <MDTypography variant="body2" sx={{ p: 1 }}>
            <CircleIcon sx={{ color: "orange" }} fontSize="small" /> Score(max):
            84{" "}
          </MDTypography>
          <MDTypography variant="body2" sx={{ p: 1 }}>
            Trans.per minute(avg): {serviceData?.tpmAvg || null}
          </MDTypography>
          <MDTypography variant="body2" sx={{ p: 1 }}>
            Req. per min(avg): 334 rpm
          </MDTypography>
          <MDTypography variant="body2" sx={{ p: 1 }}>
            Trans. error rate(avg): {serviceData?.latestErrorRate || null}
          </MDTypography>
          <MDTypography variant="body2" sx={{ p: 1 }}>
            CPU usage: 21%
          </MDTypography>
      ...
      >
        ...
        >
          <LightTooltip
            placement="left"
            title="Inspect Sent and Incoming Interactions for This Node"
          >
          ...
          <LightTooltip
            placement="left"
            title="Explore Request Latency Metrics and Insights"
          >
