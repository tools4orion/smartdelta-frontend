...
const ApexChart = () => {
	...
	const [state] = useState({
	  ...
	  const columns = [
		...
		{ Header: "Avg. Response Time", accessor: "avgResponseTime", align: "center" },
		{ Header: "Trans. per min", accessor: "transPerMin", align: "center" },
		{Header:'Error Rate', accessor: "errorRate", align: "center"}
	  ];
	  const generateRow = ({ health, name, avgResponseTime, transPerMin, errorRate }) => {
		return {
		  ...
		  avgResponseTime: (
			<MDBox display="flex">
			  <ReactApexChart options={avgResponseTime.options} series={avgResponseTime.series} type="area" height={35} width={100} />
			  <p style={{ marginLeft: '16px' }}>{avgResponseTime.value}</p>
			</MDBox>
		  ),
		  transPerMin: (
			<MDBox display="flex">
			  <ReactApexChart options={transPerMin.options} series={transPerMin.series} type="area" height={35} width={100} />
			  <p style={{ marginLeft: '16px' }}>{transPerMin.value}</p>
			</MDBox>
		  ),
		  errorRate: (
			<MDBox display="flex">
			  <ReactApexChart options={errorRate.options} series={errorRate.series} type="area" height={35} width={100} />
	  ...
	  console.log(rowss)
	  const rows = rowss.map(({ microservice, latestErrorRate, errorRateData, tpmData }, index) =>
	  generateRow({
		health: { label: 'Healthy', sx: { backgroundColor: '#4caf50', color: '#fff' } },
		name: microservice,
		avgResponseTime: { options: state.options1, series: state.series1, value: '220 ms' },
		transPerMin: { options: state.options10, series:  [{data:tpmData}], value: '132.4 tpm' },
		errorRate: { options: state.options2, series: [{data:errorRateData}], value: latestErrorRate + '%' },
	  })
	);
